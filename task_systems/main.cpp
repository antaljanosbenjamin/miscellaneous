...
template <typename TaskSystemType> void AddTasks(TaskSystemType &ts, std::vector<uint64_t> &runningTimes) {
  for (auto runningTime: runningTimes) {
    ts.async([runningTimeInNs = runningTime]() {
      ...
      //   std::cout << "Running time: |" << runningTimeInNs << "|\n";
      // }
...
int main(int argc, char **argv) {
  ...
  if (testCase == 0u) {
    ...
    elapsedTime = MeasureTaskSystem<TaskStealingTaskSystem>(runningTimes);
  }
